document.addEventListener("DOMContentLoaded", () => {
  ...
  // 🔥 Хэрвээ эд нар байхгүй бол огт ажиллахгүй
  if (!boardEl || !statusEl || !hintEl || !restartBtn) {
    console.error("Missing #board/#status/#hint/#restart in HTML");
    return;
  }
  ...
  function init(){
    ...
    render();
    setHint("Tap a piece → then tap a highlighted square.");
  }
  ...
  function render(){
    ...
    for(let r=0;r<8;r++){
      for(let c=0;c<8;c++){
        ...
        if(S.selected && S.selected.r===r && S.selected.c===c) sq.classList.add("selected");
        ...
        // ✅ Telegram дээр click заримдаа ажиллахгүй → pointerdown
        sq.addEventListener("pointerdown", onTap, {passive:false});
  ...
  function onTap(e){
    ...
    if(S.chainFrom){
      const mv=S.legal.find(m=>m.to.r===r && m.to.c===c);
  ...
  function applyMove(move){
    ...
    if(move.cap.length>0){
      const more = captureMovesFrom(move.to.r, move.to.c, p);
      if(more.length>0){
        S.selected={r:move.to.r, c:move.to.c};
        S.legal=more;
        S.chainFrom={r:move.to.r, c:move.to.c};
        render();
        setHint("Multi-capture available — continue jumping.");
        return;
      }
    }
